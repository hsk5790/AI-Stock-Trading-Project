...
class Agent:
    ...
    def __init__(self, environment, initial_balance, min_trading_price, max_trading_price):
        ...
        self.environment = environment          # Environment class의 object
    ...
    # 입력으로 들어온 epsilon의 확률로 무작위로 행동을 결정하고, 그렇지 않은 경우에 신경망을 통해 행동 결정한다.
    ...
    # 행동의 유효성 판단(살 수 있는지 없는지, 팔 수 있는지 없는지)
    ...
    # 매수 또는 매도할 주식 수 결정
    # 정책 신경망의 신뢰(confidence)가 높으면 매수, 매도 단위를 크게 한다.
    ...
    # agent가 결정한 행동을 수행
    # action : 탐험, 정책 신경망을 통해 결정한 행동으로 매수 매도를 의미하는 0또는 1의 값이다.
    # confidence : 정책 신경망을 통해 결정한 경우 행동에 대한 softmax 확률 값이다.
    def act(self, action, confidence):
        ...
        # 관망 => 보유 주식 수나 잔고에 영향을 미치지 않는다.
        # 대신 가격 변동은 있을 수 있으므로 포트폴리오 가치는 변동된다.
        elif action == Agent.ACTION_HOLD:
            self.num_hold += 1  # 관망 횟수 증가

        # 포트폴리오 가치 갱신
        # 포트폴리오 가치는 현재 잔고, 현재 주식 가격, 주식 보유 수에 의해 결정된다.
