...
# 신경망을 학습하는 과정에서 1. agent의 보유 주식 수, 2. 가치 신경망 출력,
# 3. 정책 신경망 출력, 4. 투자 행동, 5. 포트폴리오 가치 등을 시간에 따라 연속으로 보여준다.
class Visualizer:
    COLORS = ['r', 'b', 'g']
    ...
    # prepare() : Figure를 초기화하고 일봉 차트 출력하는 함수
    ...
    # plot() : 일봉 차트를 제외한 나머지 차트 출력
    def plot(self
             , epoch_str=None       # Figure 제목으로 표시할 에포크
             , num_epoches=None     # 총 수행할 에포크 수
             , epsilon=None         # 탐험률
             , action_list=None     # agent가 수행할 수 있는 전체 행동 리스트
             , actions=None         # agent가 수행한 행동 배열
             , num_stocks=None      # 주식 보유 수 배열
        ...
        with lock:
            ...
            # 차트 3. 가치 신경망
            # 가치는 장기적인 결과를 정의한다.(강화학습은 장기적인 결과를 최대화 할 수 있는 것을 목표로 한다.)
            ...
            # 차트 4. 정책 신경망
            # 정책은 agent가 어떻게 행동을 선택하는지를 정의한다.
            # 탐험을 노란색 배경으로 그리기
            # 매수는 빨간색, 매도는 파란색
            for exp_idx in exps:
            ...
            self.axes[4].fill_between(self.x, pvs, pvs_base,
                where=pvs > pvs_base, facecolor='r', alpha=0.1) # 포트폴리오 가치가 초기 자본금보다 높으면 빨간색
            self.axes[4].fill_between(self.x, pvs, pvs_base,
                where=pvs < pvs_base, facecolor='b', alpha=0.1) # 포트폴리오 가치가 초기 자본금보다 낮으면 파란색
