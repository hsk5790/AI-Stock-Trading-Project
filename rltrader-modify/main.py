...
os.environ['RLTRADER_BASE'] = 'C:\\big14\\BKST\\rltrader-modify'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--net', choices=['dnn', 'lstm', 'cnn', 'monkey'],
                        default='dnn')  # 가치 신경망과 정책 신경망 중 사용할 신경망 유형 선택
    parser.add_argument('--backend', choices=['pytorch', 'tensorflow', 'plaidml'],
                        default='pytorch')  # 백엔드로 사용할 Framework 설정
    ...
    if not os.path.isdir(output_path):          # 존재하는 directory 없으면
        os.makedirs(output_path)                # directory 만든다.
